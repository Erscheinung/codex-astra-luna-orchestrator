...
- The in-chat picker is scoped to the active root thread and its descendants; it does not merge children from other resumed/top-level sessions. If `codex agents` shows a child under another parent, resume that parent (or the child directly) with `codex resume <thread-id>` and then use `/agent`.
- If the child belongs to the active root but was spawned before this TUI was resumed, resume the parent once more so the picker can backfill descendants; the 0.154 TUI can otherwise report no agents after missing the child-start event.
